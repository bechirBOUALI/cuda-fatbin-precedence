...
architecture field stays correct.
    ...
    patch_size.py in.fatbin out.fatbin --entry 0 --padded-payload-size 200
    patch_size.py in.fatbin out.fatbin --fatbin-size 3177
    patch_size.py in.fatbin out.fatbin --append-entry other.fatbin --fatbin-size +1
"""
import argparse
import struct
import sys
...
def main():
    ...
    ap.add_argument("--entry", type=int, default=0,
                    help="which entry --padded-payload-size applies to")
    ap.add_argument("--padded-payload-size",
                    help="new declared padded payload size, absolute or +N / -N")
    ap.add_argument("--fatbin-size",
    ...
    ap.add_argument("--append-entry", metavar="FATBIN",
                    help="append entry 0 of FATBIN, header and payload, after "
                         ...
                         "fatbin_size unless --fatbin-size says otherwise")
    ap.add_argument("--append-raw", metavar="FILE",
                    help="append the raw bytes of FILE after the last entry's "
                         ...
                         "--padded-payload-size is widened to cover them")
    args = ap.parse_args()
    ...
    if args.padded_payload_size is not None:
        for idx, pos, _ehs, psz in entries(d):
            if idx == args.entry:
                new = resolve(args.padded_payload_size, psz)
    ...
    if args.fatbin_size is not None:
        new = resolve(args.fatbin_size, fsize)
        struct.pack_into("<Q", d, 8, new & 0xFFFFFFFFFFFFFFFF)
        as_int32 = struct.unpack("<i", struct.pack("<I", new & 0xFFFFFFFF))[0]
        print(f"{args.outfile}: fatbin_size {fsize} -> {new} "
